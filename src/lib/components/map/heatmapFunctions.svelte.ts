...
export function getHeatmapMetricData(orders: Order[], metric: string): Record<string, any> {
...
function setupColorScale(metric: HeatmapMetric, data: Record<string, any>) {
  ...
  switch(metric) {
    case 'orders': {
      ...
      legendData.state = { type: 'gradient', gradient: { min: 0, max: maxCount } };
      break;
    }
    case 'sales': {
      ...
      colorScale = d3.scaleSequential()
        .domain([0, maxSales])
        .interpolator(d3.interpolateBlues);
      legendData.state = { type: 'gradient', gradient: { min: '$0', max: `$${Math.round(maxSales).toLocaleString()}` } };
      break;
    }
    case 'discounts': {
      ...
      colorScale = d3.scaleSequential()
        ...
        .interpolator(d3.interpolateBlues);
      legendData.state = { type: 'gradient', gradient: { min: '0%', max: `${(maxDiscount * 100).toFixed(1)}%` } };
      break;
    }
    case 'profit': {
      ...
      legendData.state = { type: 'gradient', gradient: { min: minLabel, max: `$${Math.round(maxProfit).toLocaleString()}` } };
      break;
    }
    case 'shipping_cost': {
      ...
      colorScale = d3.scaleSequential()
        ...
        .interpolator(d3.interpolateBlues);
      legendData.state = { type: 'gradient', gradient: { min: '$0', max: `$${maxShipping.toFixed(2)}` } };
      break;
    }
    case 'quantity': {
