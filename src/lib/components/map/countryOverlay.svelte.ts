...
export function renderCountryOverlay(width:number, height:number, countryData: any[]) {
  ...
  if (showHeatmap.state && heatmapMetrics.state && Object.keys(heatmapMetrics.state).length > 0) {
    updateHeatmap(g.node(), heatmapMetrics.state, showHeatmap.state, heatmapMetric.state);
  }
